...
test("task assignment 与 task_started 在同一累计 ACK 顺序域传递 UUIDv4 身份", () => {
  ...
  const steering = pair.parent.deliverNext();
  assert.equal(steering?.kind, "accepted");
  if (steering?.kind === "accepted") {
    assert.equal(steering.task_assignment?.mode, "steer");
  }
  pair.flush();
...
test("v11 拒绝已移除的 compaction_resume 帧", () => {
  ...
  const valid = pair.child.publishTaskStarted({ task_id: TASK_ID, turn_id: TURN_ID });
  const removed = {
    ...valid,
    kind: "compaction_resume",
    payload: { generation: 1, decision: "host_idle" },
  } as unknown as SupervisorFrame;
  assert.deepEqual(pair.parent.receive(removed), {
    kind: "protocol_fault",
    error: "invalid_frame",
  });
});
