...
> 当前监督协议已经由 [ADR-0007](./0007-generation-scoped-compaction-resume.md) 破坏性升级为 `pi-subagent/7`；reply envelope 仍为第 4 版。下文的 v5/v3 版本号保留为本 ADR 作出时的历史记录。
...
`idle` 严格表示没有当前任务、压缩、恢复、未决命令、候选 final 或未确认 reply。安全快照原子拆分 `mailbox_pending_count`、`host_pending_count` 和 `reply_outbox_pending_count`，并用正交 `activity.phase` 表达处理、压缩、对账、finalizing 与挂起原因。`send_message.accepted` 只证明插件 mailbox 接纳；`wait_agent` 返回任务级 `task_completed`、`task_failed`、`task_interrupted`、`suspended` 等 outcome。v4 活动树和第 2 版 reply 不能通过 reload 热接管，必须结束旧树后重建。
