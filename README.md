...
**使用前说明：** 本插件仅提供子代理的创建、协作与生命周期管理能力，不内置子代理模板，也不预设固定工作流。首次使用前，请根据实际任务自行创建模板，按需定义代理角色、可用工具、模型与递归权限。
...
- **状态可视化**：TUI 会显示直接子代理状态，`/agent` 可查看当前会话范围内的完整代理树。
...
1. `get_agent_templates` 获取当前可用模板。
2. `spawn_agent` 使用模板 ID 创建直接子代理。
3. `send_message` 向子代理发送任务。
4. `wait_agent` 等待回复或任务结果。
5. 继续复用该子代理，或在不再需要时终止它。

`send_message` 返回 `accepted: true` 只表示任务已被接收，不表示子代理已经完成。耗时任务出现等待超时也不会被停止，应继续等待或查询状态。

同一个子代理可以继续接收新任务：

```text
继续使用“鉴权分析”子代理，根据刚才的结论检查刷新令牌流程，并补充遗漏风险。
```

它会保留此前任务的上下文，适合处理同一主题下的连续工作。
...
- 分析不同模块或不同文件；
- 调研互不依赖的技术方案；
- 分别执行代码检查、文档检查和测试结果分析；
- 从不同角度评审同一方案，但不同时修改共享文件。

存在依赖关系、会修改同一文件或共享外部资源的任务应串行执行。

### 中断与终止

- **中断**：停止当前任务，但保留子代理和上下文，之后仍可继续使用。
- **终止**：永久关闭该子代理及其后代，并释放对应名额；终止后不能复用。

需要从子代理接管任务时，应先中断并等待其停止，再由父代理继续处理，避免重复执行产生冲突。
