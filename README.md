...
交互式 TUI 会在代理工具调用行显示入参。短参数直接显示；较长的任务正文默认显示预览，展开工具行后可查看完整参数。图片参数保留类型和 MIME 信息，但 base64 数据只显示已省略的字符数，不直接输出 payload。
...
快照包含模板、名称、深度、状态、pending、修订和生命周期时间。故障节点仍返回成功外壳，并在 `data.error` 中携带稳定错误码。
