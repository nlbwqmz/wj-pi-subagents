...
所有目标共享一个观察窗口，任一目标出现有效事件时返回。工作中 `reply` 会在父会话真正接纳后唤醒等待，但只有在它进入父模型上下文后才标记为已观察；因此同一条已经被模型看到的旧 reply 不会在后续 `wait_agent` 中重复返回。任务 final、terminal 和 suspended 属于生命周期结果，按当前稳定状态裁决，不会被旧 reply 遮挡。常见 outcome：
...
- 输入 `/agent` 打开完整只读树面板。
- 节点上下文按 `占用百分比/窗口大小` 显示在时长前，例如 `33.8%/272k`；压缩后尚无可靠占用值时显示 `?/272k`。
- 节点时长按 `工作时长/生命周期时长` 显示；工作时长只累计任务处理和中断收尾，空闲、挂起及资源清理期间暂停。
