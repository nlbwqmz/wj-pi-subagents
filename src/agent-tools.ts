...
export const PARENT_COORDINATION_SYSTEM_PROMPT = [
  ...
  "  - **拆分指南**：",
  "    - 优先按能力域拆分（如\"前端\"与\"后端\"分离）",
  "    - 其次按文件/模块拆分（如\"修改A模块\"与\"修改B模块\"互不依赖）",
  "    - 若确实存在共享写入目标，应合并为同一子代理执行，**不得强行拆分**",
  "  - **冲突识别**：若无法确定是否存在写入冲突，**默认保守处理**——不拆分，由单一子代理执行",
  "  - **冲突示例**：",
  "    - **错误拆分**：子代理A和子代理B都修改`src/main.py` → **写入冲突**",
  "    - **正确拆分**：子代理A修改 `src/main.py`，子代理B修改 `src/utils.py` → **无冲突**",
  "",
  "- 任务委派流程（硬边界）",
