...
- **任务与轮次**：首条被 mailbox 接纳的父消息建立 UUID v4 `task_id`；同一任务内 steering、压缩和恢复保持该值。每次实际 Pi `agent_start` 分配新的 UUID v4 `turn_id`，child 必须先发布并获确认的 `task_started { task_id, turn_id }`，再发布该 turn reply；旧 turn final 不得提交到新 turn。中断栅栏后的消息分配后继 task，不能 steer 到正在取消的 run。
...
- **递归收尾顺序**：child runtime 的 raw `agent_settled` handler 只记录 provisional settlement 并立即返回，final 发布和 ACK 在 callback 外异步完成，因而不会阻塞同一事件上的第三方压缩 handler。压缩在 provisional settlement 后开始时撤销候选并等待新 `agent_start`；处理中压缩不要求新 turn。恢复后保持 `task_id`、更新 `turn_id`，旧候选与旧 turn final 被隔离。
