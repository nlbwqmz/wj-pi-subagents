...
- [预构资源确认边界与可注入进程树替身](issues/17-process-tree-resource-boundary.md) — 固定不透明 `ProcessTreeAdapter` 句柄、退出/资源三态观察和保守终止决策；仅整树退出与资源释放同时确认才进入 `terminated` 并释放名额，fake 以场景序列稳定复现优雅超时、孙进程残留、部分回收、重复回收和句柄释放。
